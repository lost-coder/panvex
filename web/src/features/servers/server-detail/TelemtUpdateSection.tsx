...
import { isStrictSemver, semverLt } from "./semver";
import {
  composeRestartSpec,
  isValidRestartName,
  parseRestartSpec,
...
interface Props {
  ...
  /**
   * Task 4: the checker's release list (Task 1/3), sorted descending by
   * the backend — index 0 is always "latest". Empty (or omitted) means no
   * release data at all, in which case the picker doesn't render, same as
   * the pre-Task-4 fallback when `latestVersion` was unset.
   */
...
export function TelemtUpdateSection({
  agentId,
  currentVersion,
  latestVersion,
  availableVersions = [],
  ...
  // valid entry — an in-progress pick from a background refetch must not
  // get clobbered. Adjusted during render (React's documented pattern for
  // deriving state from a prop change) rather than in an effect, so the
  // list arriving and the default selection land in the same commit.
  ...
  // Task 4 fix: the agent's own downgrade gate is fail-closed — when it
  // can't parse its *own* reported version (blank on a fresh enroll, "dev",
  // a git SHA) it has no way to know an install isn't a downgrade, so it
  // requires `allow_downgrade` for ANY install, not just an actual
  // downgrade. Comparing such a currentVersion against the picker's
  // versions is also meaningless, so the "(current)"/disabled treatment
  // is skipped entirely in this case.
  const isForced = !isStrictSemver(currentVersion);

  // Task 4: (latest)/(current) tags — current takes priority over latest
  ...
  const versionOptions = pickerVersions.map((version, index) => {
    ...
    return {
      value: version,
      label: isCurrent
        ? t("telemtUpdate.action.optionCurrent", { version })
        : index === 0
          ? t("telemtUpdate.action.optionLatest", { version })
          : version,
      disabled: isCurrent,
    };
  });
  ...
  const installDisabled =
    !!reason ||
    !selectedVersion ||
    (!isForced && selectedVersion === currentVersion) ||
    dispatchMutation.isPending;

  async function handleInstallClick() {
    // semverLt already returns false whenever either side isn't a
    // well-formed release tag, so isDowngrade stays false when isForced —
    // the two branches below are mutually exclusive.
    const isDowngrade = semverLt(selectedVersion, currentVersion);
    const ok = await confirm(
      isForced
        ...
        : isDowngrade
          ? {
              title: t("telemtUpdate.action.downgradeTitle"),
              body: t("telemtUpdate.action.downgradeBody", {
                current: currentVersion,
                version: selectedVersion,
              }),
              confirmLabel: t("telemtUpdate.action.downgradeConfirm"),
              variant: "danger",
            }
          : {
              title: t("telemtUpdate.action.confirmTitle"),
              body: t("telemtUpdate.action.confirmBody", { version: selectedVersion }),
    ...
    dispatchMutation.mutate({
      version: selectedVersion,
      ...(isForced || isDowngrade ? { allowDowngrade: true } : {}),
    });
  }

  const installButton = (
    <Button size="sm" disabled={installDisabled} onClick={handleInstallClick}>
      {dispatchMutation.isPending
        ? t("telemtUpdate.action.updating")
        : t("telemtUpdate.action.installButton", { version: selectedVersion })}
    </Button>
  );
            ...
            <div className="flex flex-col gap-2 rounded-md border border-divider bg-bg-card/50 p-3">
              {strategy?.mode === "docker" ? (
                ...
                <div className="flex flex-wrap items-end justify-between gap-2">
                  <FormField
                    label={t("telemtUpdate.action.versionLabel")}
                    variant="compact"
                  ...
                  {reason ? (
                    <Tooltip content={t(`telemtUpdate.action.disabledReason.${reason}`)}>
                      <span className="inline-flex">{installButton}</span>
                    </Tooltip>
                  ) : (
                    installButton
                  )}
