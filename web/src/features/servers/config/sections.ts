...
//
// Both directions are scoped strictly to the curated PARAM_CATALOG set:
 ...
 * { "section.key": value } map for ONLY the curated PARAM_CATALOG paths
...
export function unflattenPaths(values: Record<string, unknown>): Record<string, unknown> {
  const out: Record<string, unknown> = {};
  const byPath = new Map(PARAM_CATALOG.map((f) => [f.path, f] as const));
