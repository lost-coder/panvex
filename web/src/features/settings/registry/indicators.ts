...
export interface FieldIndicator {
  ...
  tone: IndicatorTone | null; // icon color
...
// Tailwind v4 token-driven 4px inset accent bar, keyed by tone.
...
export function resolveIndicator(schema: SchemaEntry, values: ValuesEntry): FieldIndicator {
  ...
  if (values.locked === true && (values.source === "config_file" || isConfigTier || values.source === "default" || values.source === "env")) {
