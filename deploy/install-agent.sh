...
# ── S1: release-signing public key (openssl ECDSA P-256/SHA-256) ──────────────
#
# Every release archive is signed by CI (.github/workflows/release.yml) with
# `openssl dgst -sha256 -sign` using the PANVEX_SIGNING_KEY secret. The matching
# public key embedded below is the SAME key the in-app updater pins
# (internal/security/signing_key.pub) — installer and running binary share one
# trust anchor. The detached signature is published as a `<archive>.sig` sidecar
# (raw DER). Verification runs by DEFAULT; opt out with
# PANVEX_INSTALL_SKIP_SIGNATURE=1, or override the key with
# PANVEX_INSTALL_PUBKEY_FILE=/path/to/key.pem. See deploy/release-signing.md.
...
# to verify your own builds. Runs AFTER the sha256 check so both must pass.
verify_signature() {
  local archive="$1"
  local sig_url="$2"
...
Security (release-asset signing):
  ...
  PANVEX_INSTALL_PUBKEY_FILE Path to an alternate PEM public key to verify
                             against (for self-distributed builds).
EOF
  exit 0
fi
