...
func TestIPLockoutTracker_NotLockedOnFreshKey(t *testing.T) {
	...
	if tr.IsLockedWithContext(context.Background(), "203.0.113.1", time.Now()) {
...
func TestIPLockoutTracker_LocksAfterBudget(t *testing.T) {
	...
	for i := 0; i < IPLockoutMaxFailures-1; i++ {
		tr.RecordFailureWithContext(context.Background(), "203.0.113.1", now.Add(time.Duration(i)*time.Second))
	}
	if tr.IsLockedWithContext(context.Background(), "203.0.113.1", now.Add(time.Minute)) {
		t.Fatalf("IP locked at %d failures, want > budget", IPLockoutMaxFailures-1)
	}
	// One more — must lock.
	tr.RecordFailureWithContext(context.Background(), "203.0.113.1", now.Add(time.Minute))
	if !tr.IsLockedWithContext(context.Background(), "203.0.113.1", now.Add(time.Minute)) {
...
func TestIPLockoutTracker_LockedIPRejected(t *testing.T) {
	...
	// safely inside.
	locked := tr.CheckAndRecordFailureWithContext(context.Background(), "203.0.113.1", now.Add(time.Minute))
	if !locked {
...
func TestIPLockoutTracker_DifferentIPsIndependent(t *testing.T) {
	...
	if tr.IsLockedWithContext(context.Background(), "198.51.100.1", now.Add(time.Minute)) {
...
func TestIPLockoutTracker_LockoutExpiresAfterDuration(t *testing.T) {
	...
	if !tr.IsLockedWithContext(context.Background(), "203.0.113.1", lastFail.Add(IPLockoutDuration-time.Second)) {
...
func TestIPLockoutTracker_OldFailuresDropOutOfWindow(t *testing.T) {
	...
	for i := 0; i < IPLockoutMaxFailures-1; i++ {
		tr.RecordFailureWithContext(context.Background(), "203.0.113.1", now)
	}
	// Far enough in the future that all prior stamps are pruned.
	future := now.Add(IPLockoutWindow + time.Minute)
	tr.RecordFailureWithContext(context.Background(), "203.0.113.1", future)
	// Only one failure within the window now — must NOT lock.
	if tr.IsLockedWithContext(context.Background(), "203.0.113.1", future) {
...
func TestIPLockoutTracker_LockedAttemptsDoNotExtendDeadline(t *testing.T) {
	...
	for i := 0; i < 200; i++ {
		tr.RecordFailureWithContext(context.Background(), "203.0.113.1", tripAt.Add(time.Minute+time.Duration(i)*time.Second))
	}
	// One minute past the deadline — must be unlocked.
	if tr.IsLockedWithContext(context.Background(), "203.0.113.1", deadline.Add(time.Minute)) {
...
func TestIPLockoutTracker_EmptyKeyNoOp(t *testing.T) {
	...
	tr := NewIPLockoutTracker()
	now := time.Date(2026, time.May, 3, 0, 0, 0, 0, time.UTC)
	tr.RecordFailureWithContext(context.Background(), "", now)
	if tr.IsLockedWithContext(context.Background(), "", now) {
...
func TestIPLockoutTracker_ActiveCount(t *testing.T) {
	...
	for _, ip := range []string{"203.0.113.1", "203.0.113.2"} {
		for i := 0; i < IPLockoutMaxFailures; i++ {
			tr.RecordFailureWithContext(context.Background(), ip, now.Add(time.Duration(i)*time.Second))
		}
	}
