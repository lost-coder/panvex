...
// across observer churn (Phase 4 §4.2).
...
func (s *Store) DeleteAgentFallbackState(ctx context.Context, agentID string) error {
	_, err := s.db.ExecContext(ctx, `DELETE FROM agent_fallback_state WHERE agent_id = ?`, agentID)
	return err
}
...
func (s *Store) GetAgentFallbackState(ctx context.Context, agentID string) (storage.AgentFallbackStateRecord, error) {
	row := s.db.QueryRowContext(ctx, `
		SELECT agent_id, entered_at_unix FROM agent_fallback_state WHERE agent_id = ?
	`, agentID)
	var (
		id string
		ts int64
