...
func (s *Store) RotateAgentCert(ctx context.Context, agentID string, serial string, spki []byte, overlapUntil time.Time) error {
	result, err := s.db.ExecContext(ctx, `
		UPDATE agents SET
			...
			cert_spki_sha256_prev = CASE WHEN length(cert_spki_sha256) = 0 THEN '\x'::bytea ELSE cert_spki_sha256 END,
			cert_overlap_until = CASE WHEN cert_serial = '' AND length(cert_spki_sha256) = 0 THEN NULL ELSE $1 END,
			cert_serial = $2,
			cert_spki_sha256 = $3
		WHERE id = $4
	`, overlapUntil.UTC(), serial, spki, agentID)
	if err != nil {
		return err
	}
	rowsAffected, err := result.RowsAffected()
