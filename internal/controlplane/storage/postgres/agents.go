...
// (enrollment) has nothing to fall back to, and an empty pin is what the
// verifier already treats as fail-closed.
//
	...
	result, err := s.db.ExecContext(ctx, `
		UPDATE agents SET
			cert_serial_prev = CASE
				WHEN $5 != '' AND $5 = cert_serial_prev AND cert_overlap_until IS NOT NULL THEN cert_serial_prev
				WHEN cert_serial = '' THEN ''
				ELSE cert_serial END,
			cert_spki_sha256_prev = CASE
				WHEN $5 != '' AND $5 = cert_serial_prev AND cert_overlap_until IS NOT NULL THEN cert_spki_sha256_prev
				WHEN length(cert_spki_sha256) = 0 THEN '\x'::bytea
				ELSE cert_spki_sha256 END,
			cert_overlap_until = CASE
				WHEN $5 != '' AND $5 = cert_serial_prev AND cert_overlap_until IS NOT NULL THEN cert_overlap_until
				WHEN cert_serial = '' AND length(cert_spki_sha256) = 0 THEN NULL
				ELSE $1 END,
			cert_serial = $2,
			cert_spki_sha256 = $3
		WHERE id = $4
	`, overlapUntil.UTC(), serial, spki, agentID, presentedSerial)
	if err != nil {
		return err
	}
	rowsAffected, err := result.RowsAffected()
...
func (s *Store) GetAgentCertPins(ctx context.Context, agentID string) (storage.AgentCertPins, error) {
	var (
		...
		overlap sql.NullTime
	)
	err := s.db.QueryRowContext(ctx, `
		SELECT cert_serial, cert_spki_sha256, cert_serial_prev, cert_spki_sha256_prev, cert_overlap_until
		FROM agents WHERE id = $1
	`, agentID).Scan(&pins.Serial, &pins.SPKI, &pins.PrevSerial, &pins.PrevSPKI, &overlap)
	if errors.Is(err, sql.ErrNoRows) {
...
func (s *Store) CloseAgentCertOverlap(ctx context.Context, agentID string) error {
	_, err := s.db.ExecContext(ctx, `
		UPDATE agents SET cert_serial_prev = '', cert_spki_sha256_prev = '\x'::bytea, cert_overlap_until = NULL
		WHERE id = $1
	`, agentID)
	return err
}
