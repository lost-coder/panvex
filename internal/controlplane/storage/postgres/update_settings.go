...
func (s *Store) PutUpdateSettings(ctx context.Context, data json.RawMessage) error {
	_, err := s.db.ExecContext(ctx,
		`INSERT INTO update_config (key, value) VALUES ('settings', $1) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value`,
		string(data))
	return err
}
...
func (s *Store) PutUpdateState(ctx context.Context, data json.RawMessage) error {
	_, err := s.db.ExecContext(ctx,
		`INSERT INTO update_config (key, value) VALUES ('state', $1) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value`,
		string(data))
	return err
}
