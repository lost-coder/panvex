...
func (s *Store) PutUser(ctx context.Context, user storage.UserRecord) error {
	...
	`, user.ID, user.Username, user.PasswordHash, user.Role, user.TotpSecret, user.CreatedAt.UTC())
	return err
}

func (s *Store) GetUserByID(ctx context.Context, userID string) (storage.UserRecord, error) {
	row := s.db.QueryRowContext(ctx, `
		SELECT id, username, password_hash, role, totp_secret, created_at
		FROM users
		WHERE id = $1
	`, userID)

	var user storage.UserRecord
	if err := row.Scan(&user.ID, &user.Username, &user.PasswordHash, &user.Role, &user.TotpSecret, &user.CreatedAt); err != nil {
...
func (s *Store) ListUsers(ctx context.Context) ([]storage.UserRecord, error) {
	...
	for rows.Next() {
		var user storage.UserRecord
		if err := rows.Scan(&user.ID, &user.Username, &user.PasswordHash, &user.Role, &user.TotpSecret, &user.CreatedAt); err != nil {
			return nil, err
		}
		user.CreatedAt = user.CreatedAt.UTC()
